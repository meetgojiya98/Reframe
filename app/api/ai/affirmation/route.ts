...
import { runAffirmation } from "@/lib/ai/app-features";
...
export const POST = withApiHandler(async (request: Request) => {
  ...
  if (!isAIFeatureEnabled("affirmation")) {
    return NextResponse.json({ error: "AI affirmation is not available." }, { status: 503 });
  }
